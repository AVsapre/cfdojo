...
quint32 CphFileHandler::calculateCrc32(const QByteArray &data) {
...
void CphFileHandler::addFile(const QString &name, const QByteArray &content) {
    files_[name] = content;
}

QByteArray CphFileHandler::getFile(const QString &name) const {
    return files_.value(name);
}

bool CphFileHandler::hasFile(const QString &name) const {
    return files_.contains(name);
}

QStringList CphFileHandler::fileNames() const {
    return files_.keys();
}

void CphFileHandler::clear() {
    files_.clear();
}

bool CphFileHandler::save(const QString &path) const {
    QFile file(path);
...
bool CphFileHandler::load(const QString &path) {
    QFile file(path);
    ...
    while (!file.atEnd()) {
        ...
        if (signature == kLocalFileSignature) {
            ...
            files_[QString::fromUtf8(fileName)] = content;
